...
import com.pdfreader.scanner.pdfviewer.utils.adapter.BrowserViewHolder;
...
public class CopyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private OnFolderItemWithOptionClickListener mListener;

    public CopyAdapter(OnFolderItemWithOptionClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder.getItemViewType() == 0) {
            ((CopyViewHolder) holder).bindView(position, mFileList.get(position), mCurrentItem, mListener);
        } else {
