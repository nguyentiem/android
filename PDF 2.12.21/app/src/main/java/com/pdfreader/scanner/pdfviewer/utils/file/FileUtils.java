...
////// get cac file pdf, exel , word tu external
...
//
    ...
    public static FileData getFileDataFromFilePath(String filePath){
        File file = new File(filePath);
        Uri uri = Uri.fromFile(file);
        int size = Integer.parseInt(String.valueOf(file.length()/1024));
        ...
        try {
            ...
            if (fileDescriptor != null) {
                ...
                fileData.setPages(renderer.getPageCount());
            }
        } catch (Exception ignored) {
    ...
    public static ArrayList<FileData> getFolderListOfDirectory(FileData dirFileData) {
        String dirPath = dirFileData.getFilePath();
...
public static void uploadFiles(Activity context, List<FileData> fileData){
  ShareCompat.IntentBuilder intentBuilder = ShareCompat.IntentBuilder.from(context)
            .setText("Share Document")
            .setType("application/pdf");
           for(FileData fdata:fileData) {
               File file = new File(fdata.getFilePath());
               Uri uri = FileProvider.getUriForFile(context, AUTHORITY_APP, file);
               intentBuilder.addStream(uri);
           }
           Intent uploadIntent = intentBuilder
            .getIntent();
            try {
                context.startActivity(uploadIntent);
            } catch (Exception e) {
