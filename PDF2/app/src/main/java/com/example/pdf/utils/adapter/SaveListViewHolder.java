                ...
                try {
                    AsyncTask asyncTask = new AsyncTask() {
                        ...
                        protected Object doInBackground(Object[] objects) {
                            if (PdfUtils.isPDFEncrypted(fileData.getFilePath())) {
                                try {
                                    ((Activity) itemView.getContext()).runOnUiThread(() -> {
                                        mDateTextView.setText(itemView.getContext().getString(R.string.number_page_locked));
                                    });
                            ...
                            try {
                                ((Activity) itemView.getContext()).runOnUiThread(() -> {
                                    mDateTextView.setText(itemView.getContext().getString(R.string.number_page, numberPage));
                                });
